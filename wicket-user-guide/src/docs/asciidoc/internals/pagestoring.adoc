...
At the end of the request all collected page instances are being stored in the first level cache - http session. They are stored in http session attribute named _
[wicket:persistentPageManagerData-APPLICATION_NAME] and passed to the underlying _IPageStore_.
