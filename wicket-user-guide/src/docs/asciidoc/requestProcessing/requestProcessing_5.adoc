...
To control the behavior in Ajax requests the application may use _org.apache.wicket.settings.IExceptionSettings1.  setAjaxErrorHandlingStrategy(IExceptionSettings.AjaxErrorStrategy)_. By default if an error occurs during the 
processing of an Ajax request Wicket will render the configured error page. By configuring _org.apache.wicket.settings.IExceptionSettings.  AjaxErrorStrategy1.INVOKE_FAILURE_HANDLER_ as the default strategy the application will call the JavaScript _onFailure_ callback(s) instead.
