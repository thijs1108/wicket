...
http://jqueryui.com/position/[JQueryUI library] that allows us to position DOM elements on our page relative to another element.
...
<<an-example-of-integration-with-javascript,chapter 16>>. When we built it we didn't think about a possible use of the component with AJAX.  When a complex component like our datepicker is refreshed via AJAX, the following two side effects can occur: 
